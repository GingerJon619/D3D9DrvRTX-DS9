...
// "DS9" tells the engine to read config from System\DS9.ini rather than
// a per-class ini file.  Must match the section used in the shipped ini.
IMPLEMENT_CLASS_CFG(UD3D9RenderDevice, "DS9");
...
UBOOL UD3D9RenderDevice::SetRes(INT NewX, INT NewY, INT NewColorBytes, UBOOL Fullscreen) {
	...
	// KHG-specific: push down the "Curtain" splash window
...
#else  // standard + DEEP_SPACE_NINE (DS9 uses Lock like UT)
...
void UD3D9RenderDevice::renderMover(FSceneNode* frame, ABrush* mover) {
	...
	for (std::pair<const SurfKey, std::vector<FPoly*>>& entry : polys) {
		...
		for (FPoly* poly : entry.second) {
			FSavedPoly* sPoly = (FSavedPoly*)New<BYTE>(GMem, sizeof(FSavedPoly) + poly->NumVertices * sizeof(FTransTexture*));
			sPoly->NumPts = poly->NumVertices;
			sPoly->Next = NULL;
			FTransTexture* trans = New<FTransTexture>(GMem, sPoly->NumPts);

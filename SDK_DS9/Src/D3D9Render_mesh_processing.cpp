...
void UD3D9RenderDevice::renderMeshActor(FSceneNode* frame, AActor* actor, RenderList& renderList, SpecialCoord* specialCoord) {
	...
	if (renderAsParticles) {
		...
		FPlane color(lux, lux, lux, 1.0f);
		if (GIsEditor && (baseFlags & PF_Selected)) {
			color = FPlane(color * 0.4f + FVector(0.0f, 0.6f, 0.0f), 1.0f);
		}
#if UNREAL_GOLD_OLDUNREAL
		...
		for (INT i = 0; i < numVerts; i++) {
			...
			if (actor->bRandomFrame) {
				tex = actor->MultiSkins[(INT)appCeil((&sample - samples) / 3.f) % 8];
				if (tex) {
	...
	for (INT i = 0; i < numTris; i++) {
		...
		if (isLod) {
			...
			for (int j = 0; j < 3; j++) {
				FMeshWedge& wedge = meshLod->Wedges(face.iWedge[j]);
				sampleIdx[j] = wedge.iVertex;
				triUV[j] = FMeshUV{wedge.U, wedge.V};
			}
...
#endif  // UTGLR_NO_LODMESH
		{
			...
			for (int j = 0; j < 3; j++) {
				sampleIdx[j] = tri.iVertex[j];
				triUV[j] = FMeshUV{tri.U[j], tri.V[j]};
			}
			texIdx = (INT)tri.MatIndex;
			polyFlags = (DWORD)tri.zFlags;
		}
...
void UD3D9RenderDevice::renderStaticMeshActor(FSceneNode* frame, AActor* actor, RenderList& renderList, SpecialCoord* specialCoord) {
	...
	if (renderAsParticles) {
		...
		for (INT i = 0; i < mesh->SMVerts.Num(); i++) {
			...
			if (actor->bRandomFrame) {
				tex = actor->MultiSkins[(INT)appCeil((&sample - &mesh->SMVerts(0)) / 3.f) % 8];
				if (tex) {
					tex = getTextureWithoutNext(tex, currentTime, actor->LifeFraction());
				}
			}
...
void UD3D9RenderDevice::renderSkeletalMeshActor(FSceneNode* frame, AActor* actor, RenderList& renderList, const FCoords* parentCoord) {
	...
	if (actor->bParticles) {
		...
		if (!actor->ColorAdjust.IsZero()) {
			color = FPlane(color * 0.4f + actor->ColorAdjust / 255.f * 0.6f, 1.0f);
		}
		...
		for (INT i = 0; i < numVerts; i++) {
			...
			if (actor->bRandomFrame) {
				tex = actor->MultiSkins[(INT)appCeil((&sample - deformed) / 3.f) % 8];
				if (tex) {
					tex = getTextureWithoutNext(tex, currentTime, actor->LifeFraction());
				}
			}

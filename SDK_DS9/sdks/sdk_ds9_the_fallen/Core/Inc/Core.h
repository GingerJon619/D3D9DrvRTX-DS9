...
public:
    ...
    virtual FName GetStateName() { return FName(NAME_None); }
    virtual void  InitExecution();
    virtual void  ShutdownAfterError();
    virtual void  PostEditChange();
    virtual void  CallFunction(struct FFrame& Stack, void* const Result, class UFunction* F) {}
...
// Verified against DS9 Core.dll build 338 via export + disassembly analysis.
//=============================================================================

// ENativeConstructor tag — first arg of the UClass native ctor
enum ENativeConstructor { EC_NativeConstructor };

// GPackage: each package DLL defines its own GPackage — it is NOT imported from
// Core.dll so it must NOT carry CORE_API/dllimport.  The extern "C" declaration
// here lets IMPLEMENT_CLASS_CFG reference it; IMPLEMENT_PACKAGE provides the
// definition in the same translation unit (D3D9DrvRTX.cpp).
extern "C" const ANSICHAR* GPackage;

#define DECLARE_BASE_CLASS(TClass,TSuperClass,TStaticFlags) \
public: \
    typedef TSuperClass Super; \
    typedef TClass ThisClass; \
    /* StaticClassFlags: consumed by IMPLEMENT_CLASS to pass classFlags arg */ \
...
// Verified UClass native ctor signature (ret 0x3C = 15 stack params, __thiscall):
//   ENativeConstructor, sizeof, classFlags, superClass, withinClass,
//   FGuid{A,B,C,D}, name, packageName, configName, objectFlags,
//   InternalConstructor, StaticConstructor
//=============================================================================

// IMPLEMENT_PACKAGE: defines GPackage for this DLL.
// Must NOT use CORE_API (__declspec(dllimport)) — GPackage is defined locally
// in each package DLL, not imported from Core.dll.
#define IMPLEMENT_PACKAGE(pkg) \
    static const ANSICHAR _PkgName_##pkg[] = #pkg; \
    extern "C" { const ANSICHAR* GPackage = _PkgName_##pkg; }

// IMPLEMENT_CLASS_CFG: full form with explicit config file base name.
//
// UClass native ctor ABI — verified vs. Core.dll disassembly (Build 338):
//   12 C++ params; FGuid passed by value as a struct → 4 DWORD stack slots
//   → 15 total stack slots = ret 0x3C confirmed.
//
// ClassStaticConstructor (last arg, stored at UClass+0x4E4):
//   UClass::Register() executes CALL [this+0x4E4] unconditionally unless
//   super's staticConstructor matches ours.  Passing nullptr causes a GPF
//   because Engine.dll's URenderDevice::StaticConstructor is non-null, so
//   the equality guard is never taken.
//
//   DS9 uses UTGLR_ALT_STATIC_CONSTRUCTOR: the real StaticConstructor is
//   static void(UClass*) — incompatible with void(UObject::*)().
//   TClass::StaticConstructorThunk() is a non-static member thunk that
//   Core.dll can call as void(UObject::*)(); it forwards to the real
//   static form using 'this' (which is the UClass* in that calling context).
#define IMPLEMENT_CLASS_CFG(TClass, ConfigName) \
    UClass TClass::PrivateStaticClass( \
        EC_NativeConstructor, \
        (DWORD)sizeof(TClass), \
        (DWORD)TClass::StaticClassFlags, \
        ...
        (void(UObject::*)())&TClass::StaticConstructorThunk \
    ); \
    UClass* TClass::StaticClass()              { return &PrivateStaticClass; } \
    const ANSICHAR* TClass::StaticConfigName() { return ConfigName; }

// IMPLEMENT_CLASS: config name defaults to the class name.
// Classes reading a specific ini (e.g. "DS9") use IMPLEMENT_CLASS_CFG directly.
#define IMPLEMENT_CLASS(TClass) IMPLEMENT_CLASS_CFG(TClass, #TClass)
...
public:
    ...
    // Padding to match real Build 338 sizeof(UClass) = 0x4F4 = 1268 bytes.
    //
    // Measured from Engine.dll BSS: consecutive PrivateStaticClass objects are
    // spaced 0x4F8 bytes apart (4-byte autoclass prefix + 0x4F4-byte UClass object).
    // Core.dll's UClass::Register() accesses fields at offsets up to 0x4A0 —
    // past the end of our 460-byte stub — causing the GPF on launch.
    // The UClass native constructor (in Core.dll) writes all real fields at their
    // correct offsets; we only need the object to be large enough to hold them.
    //
    // Current stub layout: vtbl(4)+UObject(44)+UField(8)+UStruct(32)+UState(276)+UClass_fields(96) = 460
    // Padding = 1268 - 460 = 808 bytes.
    BYTE _ClassPad[808];

    // Native-constructor overload — verified vs. Undying SDK UClass ctor (same ABI).
    // ret 0x3C = 15 stack *slots*, but that is 12 C++ params: FGuid is passed by
    // value and occupies 4 consecutive DWORD slots on the stack.
    UClass(ENativeConstructor,
           DWORD           ObjectSize,
           DWORD           ClassFlags,
           UClass*         SuperClass,
           UClass*         WithinClass,
           FGuid           Guid,
           const ANSICHAR* Name,
           const ANSICHAR* PackageName,
           const ANSICHAR* ConfigName,

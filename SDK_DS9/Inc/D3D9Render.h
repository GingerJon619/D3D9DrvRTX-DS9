...
class UD3D9Render : public URender {
	...
	// Thunk: same pattern as UD3D9RenderDevice — Core.dll calls this as
	// void(UObject::*)() with ECX = the UClass* being registered.
	...
	virtual FName GetStateName() override { return FName(NAME_None); }
...
private:
	...
	struct ModelFacets {
		std::array<std::array<SurfKeyBucketVector<UTexture*, SurfaceData>, RPASS_MAX>, MAX_ZONES> facetPairs;
	};
	static struct {
		ULevel* currentLevel;

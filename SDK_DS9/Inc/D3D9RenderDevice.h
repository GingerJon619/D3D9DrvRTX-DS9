...
static inline UTexture* getTextureWithoutNext(UTexture* texture, FTime time, FLOAT fraction) {
	...
	for (UTexture* next = texture->AnimNext; next && next != texture; next = next->AnimNext)
		count++;
	INT index = Clamp((INT)appFloor(fraction * count), (INT)0, (INT)(count - 1));
	while (index-- > 0)
...
#elif DEEP_SPACE_NINE
	// DS9 uses the same old UE1 CacheID layout as KHG
	INT texIndex = texInfo.CacheID >> 32;
	UObject* cacheItem = UObject::GetIndexedObject(texIndex);
	return Cast<UTexture>(cacheItem);
#else
...
#define TEX_FLAG_NO_CLAMP	0x00000001
	...
	// StaticConfigName() is provided by IMPLEMENT_CLASS_CFG(UD3D9RenderDevice, "DS9")
	// in D3D9RenderDevice.cpp — returns "DS9" so the engine reads System\DS9.ini.

	// UObject interface.
#if UTGLR_ALT_STATIC_CONSTRUCTOR
	static void StaticConstructor(UClass* clazz);
	void StaticConstructorReal();
	// Thunk: Core.dll calls the last UClass ctor arg as void(UObject::*)().
	// At that call site ECX holds the UClass* being registered, not a real
	// UD3D9RenderDevice instance.  We cast 'this' to UClass* and forward.
	void StaticConstructorThunk()
	{
		StaticConstructor(reinterpret_cast<UClass*>(this));
	}
#else
	...
	inline void FASTCALL SetTexture(INT Multi, FTextureInfo& Info, DWORD PolyFlags, FLOAT PanBias) {
		...
		// Set panning.
		Tex.UPan = Info.UPan + (PanBias * Info.UScale);
		Tex.VPan = Info.VPan + (PanBias * Info.VScale);

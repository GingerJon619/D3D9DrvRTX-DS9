...
void UD3D9Render::SurfaceData::calculateSurfaceFacet(ULevel* level, const DWORD flags) {
	...
	for (const INT& iNode : this->nodes) {
		...
		FTransTexture* transArr = New<FTransTexture>(currentLevelData.facetsMem, poly->NumPts);
		for (int i = 0; i < poly->NumPts; i++) {
			FVert& vert = model->Verts(node.iVertPool + i);
			FTransTexture* trans = transArr + i;
...
void UD3D9Render::DrawWorld(FSceneNode* frame) {
	...
	*currFrameMaxZ = -1.0f;
	...
	ASkyZoneInfo* skyZone = static_cast<ASkyZoneInfo*>(frame->Level->GetZoneActor(frame->ZoneNumber)->SkyZone);
	...
	if (!GIsEditor && playerActor && (viewport->Actor->ShowFlags & SHOW_Actors)) {
		...
		// DS9: Use standard UE1 weapon/view-model rendering path
		// The game manages its own viewmodel via eventRenderOverlays
